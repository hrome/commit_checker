...
class FunctionalTestFileSizeRestriction(unittest.TestCase):
    def setUp(self):
    ...
    def test_rejected_push(self):
        ...
        command = 'python %s --file-size-check --max-file-size-in-bytes=%s' % (self.__script_path, 20)
        ...
        self.assertEqual(p.returncode, 101, err)

    def test_pass_push(self):
        new_rev = self.__repo.parse_rev('HEAD')
        old_rev = self.__repo.parse_rev('HEAD~3')

        command = 'python %s --file-size-check --max-file-size-in-bytes=%s' % (self.__script_path, 30)
